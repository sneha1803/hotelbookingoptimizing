...
-- vi.  Lost Revneu due to Cancellations
SELECT SUM(adr) AS LossofRev, reservation_status FROM hotel_bookings GROUP BY reservation_status;


-- B] LEARNING ABOUT CANCELLATIONS
-- i. Do the Cancellations co-relate to higher room rates?
SELECT DISTINCT reservation_status, arrival_date_month, COUNT(*), AVG(adr) FROM hotel_bookings
GROUP BY arrival_date_month,reservation_status ORDER BY arrival_date_month;

-- ii. Do the Cancellations co-relate to a high waiting list?
SELECT DISTINCT reservation_status,hotel, COUNT(*), AVG(days_in_waiting_list) FROM hotel_bookings
GROUP BY hotel, reservation_status ORDER BY hotel;

-- iii. Do the Cancellations co-relate to room types?
SELECT DISTINCT  assigned_room_type,hotel, reservation_status, COUNT(*) FROM hotel_bookings
GROUP BY reservation_status, assigned_room_type,hotel ORDER BY assigned_room_type,hotel,reservation_status;
